...
namespace Dashboard.Library
{
    public class ListView : Control
    {
        ...
        private const string skListView = "ListBox"; //Control's skin we use (from skin file)
        ...
        public ListView(Manager manager, String[] _headers, float[] _weights)
            : base(manager)
        {
            ...
            CreateHeaders(_headers, _weights, Width);
            Headers.Init();
            Headers.Parent = this;
        ...
        private void DrawPane(object sender, DrawEventArgs e)
        {
            ...
            {
                SkinText font = Skin.Layers["Control"].Text;
                SkinLayer sel = Skin.Layers["Control"];
                int h = (int)font.Font.Resource.MeasureString(Items[0][0].ToString()).Y - 1;
        ...
        void DrawStrings(DrawEventArgs e, Control control, SkinLayer layer, String[] str, Rectangle rectangle, bool margins)
        {
            ...
            for (int i = 0; i < Headers.HeaderButtons.Count; i++)
            {
                ...
                e.Renderer.Draw(line, new Rectangle(rect.X - 2, rect.Y, rect.Width, 1), Color.White);
                e.Renderer.DrawString(control, layer, " " + str[i], rect, margins);
                e.Renderer.Draw(line, new Rectangle(rect.X - 2, rect.Y+rect.Height, rect.Width, 1), Color.White);
                e.Renderer.Draw(line, new Rectangle(rect.X - 2, rect.Y, 1, rect.Height), Color.White);
            }
        }
        ...
        private void FormatHeaders(Header _header)
        {
            ...
            {
                _header.HeaderButtons[i].Left = pane.Left + lastLeft;
                lastLeft += _header.HeaderButtons[i].Width;
                //_header.HeaderButtons[i].Top = pane.Top;
            }
        }

        private void CreateHeaders(string[] _headers, float[] weights, int _width)
        {
            if (_headers.Length != weights.Length) throw new ArgumentException();
            ...
            for(int i = 0;i<_headers.Length;i++)
            {
                ...
                Headers.HeaderButtons[i].Weight = weights[i];
            }
        }
        ...
        private void RescaleHeaders()
        {
            ...
            {
                h.Width = (int)(h.Weight * Headers.Width);
                h.Left = prevLeft;
                prevLeft += h.Width;
            }
        }
